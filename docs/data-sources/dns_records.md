...
* `alternative_cname_dnssec` The alternative CNAME in the DNSSEC-signed zone (`ax4z-s.com`) that points to the record. Empty when no signed alias exists, for example on reverse mapping domains.
