...
func TestSSLProviderCredentialsCustomizeDiff(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The key is generated by another resource (e.g. tls_private_key) and unknown at plan time.
			name: "cert with private key from another resource",
			config: map[string]any{
				"name":                 "D-Trust",
				"certificate_provider": "DTRUST",
				"cert":                 "-----BEGIN CERTIFICATE-----",
				"private_key":          unknownVariableValue,
				"eab_kid":              "kid",
				"eab_hmac":             "hmac",
			},
		},
		{
			// The certificate is issued by another resource (e.g. tls_self_signed_cert) and unknown at plan time.
			name: "private key with cert from another resource",
			config: map[string]any{
				"name":                 "D-Trust",
				"certificate_provider": "DTRUST",
				"cert":                 unknownVariableValue,
				"private_key":          "-----BEGIN PRIVATE KEY-----",
				"eab_kid":              "kid",
				"eab_hmac":             "hmac",
			},
